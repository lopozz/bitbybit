...
def main():
    ...
    apply_lora_to_linear_modules(
        model, target_modules=["mlp_layer", "attn_layer"], r=8, alpha=32
    )

    total = sum(p.numel() for p in model.parameters())
    trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(
        f"total params: {total / 1e6:.3f} M, trainable params: {trainable / 1e6:.3f} M"
    )
    ...
    print(f"LoRA training took {end - start}")
